...
protected abbrev Semicategory.infer (s : SemicategorySig β) [DOpAssoc s.op] : Semicategory s where
...
protected abbrev CancelSemicategory.infer (s : SemicategorySig β) [DOpAssoc s.op] [DOpLeftCancel s.op] [DOpRightCancel s.op] : CancelSemicategory s where
  dop_assoc := dop_assoc _
