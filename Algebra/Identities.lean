...
abbrev hom_ct {ct₁ ct₂} [self : HomCt hom ct₁ ct₂] := self.hom_ct

class HomFn : Prop where
  protected hom_fn (x) : hom (fn₁ x) = fn₂ (hom x)
abbrev hom_fn {fn₁ fn₂} [self : HomFn hom fn₁ fn₂] := self.hom_fn

class HomOp : Prop where
  protected hom_op (x y) : hom (op₁ x y) = op₂ (hom x) (hom y)
abbrev hom_op {op₁ op₂} [self : HomOp hom op₁ op₂] := self.hom_op
...
abbrev fn_fix {ct} [self : FnFix fn ct] := self.fn_fix

class FnComm : Prop where
  protected fn_comm (x) : fn (fn' x) = fn' (fn x)
abbrev fn_comm {fn'} [self : FnComm fn fn'] := self.fn_comm

class FnLeftInv : Prop where
  protected fn_left_inv (x) : fn' (fn x) = x
abbrev fn_left_inv {fn'} [self : FnLeftInv fn fn'] := self.fn_left_inv

class FnRightInv : Prop where
  protected fn_right_inv (x) : fn (fn' x) = x
abbrev fn_right_inv {fn'} [self : FnRightInv fn fn'] := self.fn_right_inv

class FnIdem : Prop where
  protected fn_idem (x) : fn (fn x) = fn x
abbrev fn_idem [self : FnIdem fn] := self.fn_idem

class FnInvol : Prop where
  protected fn_invol (x) : fn (fn x) = x
abbrev fn_invol [self : FnInvol fn] := self.fn_invol
...
abbrev op_assoc [self : OpAssoc op] := self.op_assoc

class OpComm : Prop where
  protected op_comm (x y) : x ⋆ y = y ⋆ x
abbrev op_comm [self : OpComm op] := self.op_comm

class OpLeftComm : Prop where
  protected op_left_comm (x y z) : x ⋆ (y ⋆ z) = y ⋆ (x ⋆ z)
abbrev op_left_comm [self : OpLeftComm op] := self.op_left_comm

class OpRightComm : Prop where
  protected op_right_comm (x y z) : (x ⋆ y) ⋆ z = (x ⋆ z) ⋆ y
abbrev op_right_comm [self : OpRightComm op] := self.op_right_comm

class OpCrossComm : Prop where
  protected op_cross_comm (x₁ x₂ y₁ y₂) : (x₁ ⋆ x₂) ⋆ (y₁ ⋆ y₂) = (x₁ ⋆ y₁) ⋆ (x₂ ⋆ y₂)
abbrev op_cross_comm [self : OpCrossComm op] := self.op_cross_comm

class OpLeftId : Prop where
  protected op_left_id (x) : e ⋆ x = x
abbrev op_left_id {id} [self : OpLeftId op id] := self.op_left_id

class OpRightId : Prop where
  protected op_right_id (x) : x ⋆ e = x
abbrev op_right_id {id} [self : OpRightId op id] := self.op_right_id

class OpLeftInv : Prop where
  protected op_left_inv (x) : x⁻¹ ⋆ x = e
abbrev op_left_inv {inv id} [self : OpLeftInv op inv id] := self.op_left_inv

class OpRightInv : Prop where
  protected op_right_inv (x) : x ⋆ x⁻¹ = e
abbrev op_right_inv {inv id} [self : OpRightInv op inv id] := self.op_right_inv

class OpLeftNil : Prop where
  protected op_left_nil (x) : nil ⋆ x = nil
abbrev op_left_nil {nil} [self : OpLeftNil op nil] := self.op_left_nil

class OpRightNil : Prop where
  protected op_right_nil (x) : x ⋆ nil = nil
abbrev op_right_nil {nil} [self : OpRightNil op nil] := self.op_right_nil

class OpLeftCancel : Prop where
  protected op_left_cancel (x) {y z} : x ⋆ y = x ⋆ z → y = z
abbrev op_left_cancel [self : OpLeftCancel op] := self.op_left_cancel

class OpRightCancel : Prop where
  protected op_right_cancel (x) {y z} : y ⋆ x = z ⋆ x → y = z
abbrev op_right_cancel [self : OpRightCancel op] := self.op_right_cancel

class OpLeftDistrib : Prop where
  protected op_left_distrib (x y z) : x ⋆ (y ⊹ z) = x ⋆ y ⊹ x ⋆ z
abbrev op_left_distrib {op'} [self : OpLeftDistrib op op'] := self.op_left_distrib

class OpRightDistrib : Prop where
  protected op_right_distrib (x y z) : (x ⊹ y) ⋆ z = x ⋆ z ⊹ y ⋆ z
abbrev op_right_distrib {op'} [self : OpRightDistrib op op'] := self.op_right_distrib
...
abbrev inv_op {op} [self : InvOp inv op] := self.inv_op

class InvHom : Prop where
  protected inv_hom (x y) : inv (op x y) = op (inv x) (inv y)
abbrev inv_hom {op} [self : InvHom inv op] := self.inv_hom

class InvInvol : Prop where
  protected inv_invol (x) : inv (inv x) = x
abbrev inv_invol [self : InvInvol inv] := self.inv_invol

class InvId : Prop where
  protected inv_id : inv id = id
abbrev inv_id {id} [self : InvId inv id] := self.inv_id
...
abbrev dinv_op {op} [self : DInvOp inv op] := self.dinv_op

class DInvInvol : Prop where
  protected dinv_invol {{a b}} (x : β a b) : inv (inv x) = x
abbrev dinv_invol [self : DInvInvol inv] := self.dinv_invol

class DInvId : Prop where
  protected dinv_id {{a}} : inv (id : β a a) = id
abbrev dinv_id {id : {a : α} → β a a} [self : DInvId inv id] := self.dinv_id
